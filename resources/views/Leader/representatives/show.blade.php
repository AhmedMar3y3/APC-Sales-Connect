...
<div class="container">
    ...
    <a href="{{ route('leader.representatives.visitCreate', $representative->id) }}" class="btn btn-success mb-3">Create New Visit</a>
            ...
            <tbody>
                @foreach($representative->visits as $visit)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('leader.representatives.visits', ['representativeId' => $representative->id, 'visitId' => $visit->id]) }}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i> View</a>
                            <a href="{{ route('leader.representatives.visits.edit', ['representativeId' => $representative->id, 'visitId' => $visit->id]) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i> Edit</a>
                            <form action="{{ route('leader.representatives.visits.destroy', ['representativeId' => $representative->id, 'visitId' => $visit->id]) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
